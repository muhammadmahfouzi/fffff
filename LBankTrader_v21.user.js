...
(() => {
  ...
  // v21: raised from 1.50 → 5.0 USDT.
  // LBank returns 10008 ("currency pair nonsupport") when the order amount is below their minimum,
  // even for valid pairs. Many pairs have minTranQua that maps to >$5 USDT. Using a floor of 5 USDT
  // prevents most below-minimum rejections when accuracy.do returns minTranQua=0 for a pair.
  // For BTC/USDT the real minimum is ~$10 USDT; accuracy.do parsing fixes below should catch that.
  const MIN_USDT_ORDER = 5.0;
...
// v20/v21: When accuracy.do failed or returned minQty=0, apply a conservative safety floor so
// we never send a below-minimum order to LBank. LBank returns 10008 for orders below their minimum,
// even for valid pairs — so block here with a clear message rather than let LBank silently reject.
// Real minimums: BTC/USDT ≈ $10–$15 USDT, most altcoins $1–$5 USDT.
const LBANK_MIN_ORDER_FALLBACK_USDT = 15; // when accuracy.do failed completely
const LBANK_MIN_ORDER_UNKNOWN_USDT = 10;  // when accuracy.do succeeded but minTranQua was 0/missing
...
function safeMinUsdtOrderValue(price, pairRules = null) {
  ...
  // Determine floor: real data > unknown (0) > api failure > base floor
  let safeFallback = MIN_USDT_ORDER; // default: $5
  if (pairRules?.accuracyFailed) {
    safeFallback = LBANK_MIN_ORDER_FALLBACK_USDT; // accuracy.do threw: $15 fallback
  } else if (pairRules?.minQtyUnknown) {
    safeFallback = LBANK_MIN_ORDER_UNKNOWN_USDT;  // accuracy.do OK but minTranQua=0: $10 fallback
  }
      ...
      if (!orderPlaced && isCurrencyPairUnsupportedMessage(String(lastErr?.message || lastErr))) {
        ...
        try {
          ...
          if (!isAllowedUsdPairInfoExact(info, cleanBase, cleanQuote)) {
            ...
            // LBank also returns 10008 when the order AMOUNT is below their minimum for the pair.
            // Compute the order's USDT value to include in the diagnostic.
            const sentUsdtAmount = side === 'buy'
              ? num(preflightPayload.amount || preflightPayload.price)
              : 0;
            const minUsdt = safeMinUsdtOrderValue(priceHint, pairRules);
            const amountHint = sentUsdtAmount > 0
              ? ` | مبلغ ارسال‌شده: ${sentUsdtAmount.toFixed(2)} USDT | حداقل امن اسکریپت: ${minUsdt.toFixed(2)} USDT`
              : '';
            // v21: primary suspect is order-amount below LBank minimum (10008 is LBank's catch-all for this)
            const diagHint =
              'جفت معاملاتی در لیست مجاز LBank تأیید شد ولی سفارش رد شد.' + amountHint + ' ' +
              'احتمالاً: (۱) مبلغ سفارش کمتر از حداقل LBank برای این جفت است — بودجه را به حداقل ۱۰ USDT افزایش بده؛ ' +
              '(۲) کلید API اجازه Spot Trading ندارد — در پنل LBank بررسی کن؛ ' +
              '(۳) محدودیت IP روی کلید API تنظیم شده — IP مرورگر را در whitelist بگذار یا محدودیت را بردار؛ ' +
              '(۴) مشکل گذرا در سمت سرور LBank — چند لحظه دیگر دوباره امتحان کن.';
            log(diagHint, 'warn');

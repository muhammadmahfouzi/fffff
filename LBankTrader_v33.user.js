...
async function buildTodayRealizedPnlReport() {
  ...
  try {
    for (let page = 1; page <= 3; page++) {
      ...
      for (const o of list) {
        const t = num(o.orderTime || o.createTime || o.updateTime || o.time || 0);
        if (t > 0 && t < fromMs) { truncated = true; break; } // passed today's window
        rawOrders.push(o);
      }
  ...
  for (const o of rawOrders) {
    ...
    if (!(orderTime >= fromMs && orderTime <= toMs)) continue;

...
(() => {
  ...
  /******************************************************************
   * v23: Token Collector — runs on www.lbank.com to grab session token
   * The main bot continues on web.bale.ai using the stored token to
   * bypass the public-API 10008 restriction via the internal ccapi.
   ******************************************************************/
  ...
  // v22: lowered back to 0.80 USDT. Verified directly against LBank's API with live keys:
  // accuracy.do works and returns the REAL per-pair minimum (minTranQua), e.g. mori_usdt=30 tokens
  // (~$0.32), at_usdt=5 tokens (~$0.60). The v21 theory that 10008 was caused by below-minimum
  // amounts was WRONG: 10008 is returned for EVERY order (any pair, market & limit, even the
  // no-cost create_order_test.do endpoint) when the API key/account is not permitted to trade.
  // Real per-pair minimums are computed from accuracy.do (minQty*price) — this floor is only a
  // tiny safety net so a $5.5 budget can place several small safe buys.
  const MIN_USDT_ORDER = 0.80;
...
async function buildManualSellCandidates(wallets, preferredQuote = 'usdt', options = {}) {
  ...
  for (const [currency, wallet] of Object.entries(map)) {
    ...
    for (const allowed of allowedPairs) {
      ...
      // v35: show ALL non-dust balances regardless of exchange minimum.
      // For high-price assets like BTC, a $1 balance is < LBank's minQty ($10+),
      // but the user must still be able to see and attempt to sell it.
      // Minimum gate is 0.001 USDT to avoid absolute zero/rounding noise only.
      if (!(quoteValue >= 0.001)) continue;
    ...
    try {
      ...
      for (let i = 0; i < selected.length; i++) {
        ...
        await executeSell(item.base, item.quote, item.balance, false, item.balance, {
          accountId,
          keepReserve: false,
          ignoreMinSize: true,  // v35: show+attempt even when balance < exchange minimum (e.g. $1 BTC)
...
// ==========================================================================
// v23: LBank Internal Web API (ccapi.rerrkvifj.com) — Session-based auth
// Bypasses the public API 10008 trading restriction using the session token
// stored by the token-collector mode (running on www.lbank.com).
// ==========================================================================
...
function httpRequestCcapi(path, data = null, skipTokenCheck = false) {
  return new Promise(async (resolve, reject) => {
    ...
    let sig, timestamp;
    try {
      const s = await ccapiComputeSignature(method, path);
      sig = s.signature;
    ...
    GM_xmlhttpRequest({
      ...
      onload: (res) => {
        ...
        if (res.status < 200 || res.status >= 300) {
          reject(new Error(`ccapi HTTP ${res.status}: ${res.responseText?.slice(0,200)}`));
          return;
        }
...
async function cancelOrderCcapi(orderId, symbol) {
  ...
  throw new Error(`[ccapi] لغو سفارش خطا (${code}): ${res.json?.message ?? res.text}`);
}
...
async function getLbankPairRules(base, quote = 'usdt', options = {}) {
  ...
  try {
    ...
    // v21 fix: LBank's accuracy.do can return either an array or a single object for symbol queries.
    // Previous code set row=null for single-object responses → minQty=0 → minimum check bypassed.
    let dataItems;
    if (Array.isArray(res?.data)) {
      dataItems = res.data;
    } else if (res?.data && typeof res.data === 'object' && !Array.isArray(res.data)) {
      // Single-object response: wrap it so the find/[0] logic below works uniformly
      dataItems = [res.data];
    } else {
      dataItems = [];
    }
    const row = dataItems.find(item => String(item?.symbol || '').toLowerCase() === symbol) || dataItems[0] || null;
    const minQtyParsed = Math.max(0, num(row?.minTranQua));
    const rules = {
      symbol,
      minQty: minQtyParsed,
      ...
      raw: row || null,
      // v21: flag when accuracy.do returned data but minTranQua was 0 or missing
      minQtyUnknown: minQtyParsed === 0,
    };
    // v21: always log what accuracy.do returned so minimum check behavior is visible
    log(
      `accuracy.do ${symbol}: items=${dataItems.length} | rowFound=${!!row} | minTranQua=${row?.minTranQua ?? 'N/A'} → minQty=${rules.minQty}` +
      (rules.minQtyUnknown ? ' ⚠ minQty=0 — از حداقل جایگزین استفاده می‌شود' : ''),
      'info'
    );
...
// v22: accuracy.do works reliably (verified live) and returns the real per-pair minimum, so the
// primary minimum is pairMinValue = minQty*price. The fallbacks below only apply when accuracy.do
// could not give a number, and are kept small so they never block a legitimate small buy.
const LBANK_MIN_ORDER_FALLBACK_USDT = 2.0; // accuracy.do threw an error
const LBANK_MIN_ORDER_UNKNOWN_USDT = 1.5;  // accuracy.do returned but minTranQua was 0/missing
...
function safeMinUsdtOrderValue(price, pairRules = null) {
  ...
  // Real per-pair minimum (from accuracy.do) is authoritative; fallbacks only for missing data.
  let safeFallback = MIN_USDT_ORDER;
  if (pairRules?.accuracyFailed) {
    safeFallback = LBANK_MIN_ORDER_FALLBACK_USDT;
  } else if (pairRules?.minQtyUnknown) {
    safeFallback = LBANK_MIN_ORDER_UNKNOWN_USDT;
  }
...
function ccapiOrderTime(o) {
  // LBank queryHistory may return orderTime as ms, seconds, or null.
  // Try all known timestamp fields; auto-upgrade seconds → ms.
  const raw = num(o.orderTime || o.createTime || o.updateTime || o.finishTime || o.time || 0);
  if (raw <= 0) return 0;
  // Unix seconds are < 10 billion (year ~2286); ms are > 10 billion today
  return raw < 10_000_000_000 ? raw * 1000 : raw;
}

async function buildTodayRealizedPnlReport() {
  const range = tehranRangeFromMidnight();
  const { fromMs, toMs } = range;

  // Fetch up to 3 pages (300 orders) from ccapi history, filter to today's Tehran window
  const rawOrders = [];
  let truncated = false;
  try {
    for (let page = 1; page <= 3; page++) {
      const res = await httpRequestCcapi(
        `/spot-trade-center/order/queryHistory?pageNo=${page}&pageSize=100`
      );
      const list = res.json?.data?.resultList || res.json?.data || [];
      if (!Array.isArray(list) || list.length === 0) break;
      for (const o of list) {
        const t = ccapiOrderTime(o);
        // t=0 means no timestamp — keep it (assume recent); t<fromMs means older than today
        if (t > 0 && t < fromMs) { truncated = true; break; }
        rawOrders.push(o);
      }
      if (truncated || list.length < 100) break;
    }
  } catch (_) {}

  // Normalize each raw order into a typed trade object
  const trades = [];
  for (const o of rawOrders) {
    const orderTime = ccapiOrderTime(o);
    // If no timestamp at all, treat as "now" (recent orders lack timestamp right after fill)
    const effectiveTime = orderTime > 0 ? orderTime : Date.now();
    if (!(effectiveTime >= fromMs && effectiveTime <= toMs + 60_000)) continue;
    ...
    const pairParts = pairCode.split('_');
    const base  = (pairParts[0] || pairCode).toUpperCase();
    const quote = normalizeQuoteCode(pairParts[1] || 'usdt');
    ...
    const recorded = num(o.amount || o.quantity || 0);
    const qty      = dealBase > 0 ? dealBase
      : (isMarketBuy && price > 0 ? recorded / price : recorded);
    const total    = num(o.dealMoney || o.cummulativeQuoteQty || 0)
      || (isMarketBuy ? recorded : qty * price);
  ...
  // Sort oldest-first for FIFO lot matching
  trades.sort((a, b) => a.timeMs - b.timeMs);

  // FIFO lot matching — identical logic to Nobitex
  const lotsByPair = {};
  const closedPairs = [];
  let unmatchedSellCount = 0;

  for (const trade of trades) {
    const key = posKey(trade.base, trade.quote);
    if (!lotsByPair[key]) lotsByPair[key] = [];
    const lots = lotsByPair[key];
    ...
    if (trade.type === 'buy') {
      lots.push({
        ...
        remainingCost: trade.total,
        buyTimeMs: trade.timeMs,
        uuid: trade.uuid,
      });
      continue;
    }
    ...
    const netProceeds = trade.total;

    while (qtyLeft > 1e-12 && lots.length) {
      const lot = lots[0];
      const matchedQty = Math.min(qtyLeft, Math.max(0, lot.remainingQty));

      if (!(matchedQty > 0)) { lots.shift(); continue; }

      const costAlloc     = Math.max(0, lot.remainingCost) * (matchedQty / Math.max(lot.remainingQty, 1e-12));
      const proceedsAlloc = netProceeds * (matchedQty / Math.max(trade.qty, 1e-12));
      const pnl           = proceedsAlloc - costAlloc;
      ...
      closedPairs.push({
        ...
        sellTimeMs: trade.timeMs,
        pnl,
        pnlPercent: calcPnlPercent(pnl, costAlloc),
        cost: costAlloc,
        proceeds: proceedsAlloc,
      });

      lot.remainingQty  = Math.max(0, lot.remainingQty  - matchedQty);
      lot.remainingCost = Math.max(0, lot.remainingCost - costAlloc);
      qtyLeft           = Math.max(0, qtyLeft - matchedQty);

      if (!(lot.remainingQty > 1e-12)) lots.shift();
    }

    if (qtyLeft > 1e-12) unmatchedSellCount += 1;
  }

  // Open lots: buys not yet matched to a sell today
  const openLots = [];
  for (const [, lots] of Object.entries(lotsByPair)) {
    const active = lots.filter(l => l.remainingQty > 1e-12);
    if (!active.length) continue;
    const first = active[0];
    openLots.push({
      base: String(first.base || '').toUpperCase(),
      quote: String(first.quote || '').toLowerCase(),
      qty: active.reduce((s, l) => s + l.remainingQty, 0),
      cost: active.reduce((s, l) => s + l.remainingCost, 0),
      buyTimeMs: Math.min(...active.map(l => l.buyTimeMs).filter(Boolean)),
    });
  }

  const totals = { usdt: { pnl: 0, cost: 0 } };
  for (const p of closedPairs) {
    const q = p.quote === 'usdt' ? 'usdt' : 'usdt';
    totals[q].pnl  += p.pnl;
    totals[q].cost += Math.max(0, p.cost);
  }

  return {
    ...range,
    closedPairs,
    openLots: openLots.sort((a, b) => (a.buyTimeMs - b.buyTimeMs) || a.base.localeCompare(b.base)),
    totals,
    truncated,
    unmatchedSellCount,
  };
...
function renderTodayRealizedPnlReport(report) {
  // Group closed pairs by (base, quote) — identical to Nobitex pattern
  const closedGroups = {};
  for (const pair of report.closedPairs || []) {
    const key = posKey(pair.base, pair.quote);
    if (!closedGroups[key]) {
      closedGroups[key] = { base: pair.base, quote: pair.quote, totalPnl: 0, totalCost: 0, items: [] };
    }
    closedGroups[key].totalPnl  += pair.pnl;
    closedGroups[key].totalCost += Math.max(0, pair.cost);
    closedGroups[key].items.push(pair);
  }

  const groupedItems = Object.values(closedGroups)
    .sort((a, b) => a.base.localeCompare(b.base) || a.quote.localeCompare(b.quote))
    .map(g => ({ ...g, totalPercent: g.totalCost > 0 ? calcPnlPercent(g.totalPnl, g.totalCost) : 0 }));

  // Summary per quote
  const summaryParts = [];
  for (const quote of ['usdt']) {
    const qGroups = groupedItems.filter(g => g.quote === quote);
    if (!qGroups.length) continue;
    const totalPnl     = qGroups.reduce((s, g) => s + g.totalPnl, 0);
    const summaryPct   = calcSummaryPercentFromValues(qGroups.map(g => g.totalPercent));
    summaryParts.push(`جمع ${quoteLabelFa(quote)}: ${formatDailyPnlSignedValue(totalPnl, quote)} (${formatSignedPercent(summaryPct)})`);
  }

  const warningLines = [];
  if (report.truncated) warningLines.push('تاریخچه به ۳۰۰ ردیف محدود شد؛ ممکن است سفارشات قدیمی‌تر امروز حذف شده باشند.');
  if (report.unmatchedSellCount > 0) warningLines.push(`${formatFaNumber(report.unmatchedSellCount)} فروش بدون خرید متناظر امروز یافت شد و در سود/زیان بسته‌شده لحاظ نشد.`);

  // Per-pair closed cards
  let groupsHtml = '';
  for (const group of groupedItems) {
    const tradeLines = group.items
      .sort((a, b) => (a.sellTimeMs - b.sellTimeMs) || (a.buyTimeMs - b.buyTimeMs))
      .map(item => {
        const line = `${formatTimeHmTehran(item.buyTimeMs)} ← ${formatTimeHmTehran(item.sellTimeMs)} | مقدار: ${formatAmount(item.qty)}`;
        return `<div style="margin-top:6px; white-space:pre-wrap;">${escapeHtml(line)}</div>`;
      }).join('');

    groupsHtml += `
      <div class="lbk-sell-item">
        <div style="font-weight:700;">${escapeHtml(group.base)} (${escapeHtml(quoteLabelFa(group.quote))}) | جمع: ${escapeHtml(formatDailyPnlSignedValue(group.totalPnl, group.quote))} (${escapeHtml(formatSignedPercent(group.totalPercent))})</div>
        ${tradeLines}
      </div>`;
  }

  // Open lots (bought today, not yet sold today)
  let openLotsHtml = '';
  if ((report.openLots || []).length) {
    const rows = report.openLots.map(item => {
      const line = `${item.base} (${quoteLabelFa(item.quote)}) | از ${formatTimeHmTehran(item.buyTimeMs)} باز مانده | مقدار تقریبی: ${formatAmount(item.qty)}`;
      return `<div style="margin-top:6px; white-space:pre-wrap;">${escapeHtml(line)}</div>`;
    }).join('');
    openLotsHtml = `
      <div style="margin-top:12px; font-weight:700;">خریدهای بازِ بدون فروش امروز</div>
      <div class="lbk-sell-list" style="margin-top:8px;">${rows || '<div>موردی ندارد.</div>'}</div>`;
  }

  const summaryText = summaryParts.length ? summaryParts.join(' | ') : 'جمع بسته‌شده‌ای برای امروز ثبت نشده است.';
  const bodyHtml = `
    <div class="lbk-summary">
      بازه امروز تهران: <b>${escapeHtml(report.fromLabel)}</b> تا <b>${escapeHtml(report.toLabel)}</b><br />
      تعداد جفت‌ها: <b>${escapeHtml(formatFaNumber(groupedItems.length))}</b><br />
      ${escapeHtml(summaryText)}
    </div>
    ${warningLines.length ? `<div style="margin-bottom:12px; color:#92400e; line-height:1.8;">${warningLines.map(l => `• ${escapeHtml(l)}`).join('<br />')}</div>` : ''}
    <div style="font-weight:700; margin-bottom:8px;">معاملات بسته‌شده</div>
    <div class="lbk-sell-list">
      ${groupsHtml || '<div style="white-space:pre-wrap;">امروز هنوز خرید/فروش بسته‌شده‌ای برای نمایش پیدا نشد.</div>'}
    </div>
    ${openLotsHtml}
  `;

  return {
    title: `لیست سود/زیان امروز (${report.toLabel})`,
    bodyHtml,
  };
}
  ...
  async function showDailyRealizedPnlPopup() {
    try {
      ...
      const popup = renderTodayRealizedPnlReport(report);
...
async function placeMarketOrder(side, base, quote, amount, priceHint, options = {}) {
  ...
  for (const candidateAmount of candidates) {
    ...
    try {
      ...
      // All style variants exhausted and still 10008 —
      // v23: First try the LBank internal web API (ccapi) using session token,
      // then fall back to diagnostics.
      if (!orderPlaced && isCurrencyPairUnsupportedMessage(String(lastErr?.message || lastErr))) {
        ...
        if (pairValid) {
          // v23: Attempt ccapi (internal LBank web API) with session token.
          // The internal API bypasses the public-API 10008 restriction because it uses
          // session-based auth rather than RSA API keys.
          const ccapiToken = getCcapiSessionToken();
          const symbol = lbankSymbol(cleanBase, cleanQuote);
          const priceAcc = Math.max(0, Math.min(18, num(pairRules?.priceAccuracy) || 6));
          const qtyAcc   = Math.max(0, Math.min(18, num(pairRules?.quantityAccuracy) || 0));
          ...
          if (ccapiToken) {
            ...
            try {
              ...
              const order = {
                ...
                price: num(priceHint),
                symbol,
                raw: { data: ccapiOrderData, _via: 'ccapi' },
                orderEndpoint: '/spot-trade-center/order/place',
              };
              return { order, amount: order.amount };

            } catch (ccapiErr) {
              const ccapiMsg = String(ccapiErr?.message || ccapiErr || '');
              log(`[ccapi] مسیر داخلی هم خطا داد: ${ccapiMsg}`, 'warn');
              lastErr = new Error(`[ccapi خطا] ${ccapiMsg}`);
            }
          }

          // ccapi unavailable or failed — show actionable diagnostic
          const tokenStatus = ccapiToken
            ? 'token ccapi موجود بود ولی سفارش رد شد — دوباره به www.lbank.com برو تا token تازه شود'
            : 'token ccapi موجود نیست — یک‌بار در مرورگر به www.lbank.com برو (لاگین‌بودن ضروری است) تا token جمع شود';
          const diagHint =
            '⚡ v23: API عمومی LBank با ۱۰۰۰۸ مسدود است و مسیر داخلی ccapi نیز امتحان شد. ' +
            tokenStatus + '. ' +
            'مراحل فعال‌سازی ccapi: ' +
            '① در مرورگر به www.lbank.com بروید و وارد حساب خود شوید (اسکریپت به‌صورت خودکار session token را ذخیره می‌کند)؛ ' +
            '② سپس در پنجره Bale دوباره سفارش بدهید تا اسکریپت از مسیر داخلی استفاده کند؛ ' +
            '③ اگر ccapi هم رد کرد، پاسخ کامل را در لاگ‌ها ببینید و گزارش دهید.';
          log(diagHint, 'warn');
  ...
  try {
    ...
    if (effectiveWalletQty !== null) {
      if (keepReserve) {
        ...
        // v35: use pair precision for floor, not global AMOUNT_DECIMALS
        sellQty = amountFloor(Math.min(desiredQty, effectiveWalletQty), quantityDecimalsForPair(pairRules));
        if (!sellQty || sellQty <= 0) {
    ...
    // v35: for manual sell (ignoreMinSize) bypass exchange-minimum gates and let the
    // order reach the exchange — reject there if needed. Applies to high-price assets
    // like BTC where a $1 balance is legitimately below LBank's $10 minimum order.
    const ignoreMinSize = !!options.ignoreMinSize;
    if (!ignoreMinSize) {

...
function renderTodayRealizedPnlReport(report) {
  ...
  // Separate history orders from open positions
  const histRows = rows.filter(r => r.status === 'history_buy' || r.status === 'history_sell');
  const openRows = rows.filter(r => r.status === 'open_local_position');

  // Build history cards
  let histHtml = '';
  for (const row of histRows) {
    const isBuy  = row.status === 'history_buy';
    const qty    = row.buyQty || row.sellQty;
    const price  = row.buyPrice || row.sellPrice;
    const value  = row.buyGrossQuote || row.sellGrossQuote;
    const quote  = row.quote || 'usdt';
    const typeLabel = isBuy ? 'خرید' : 'فروش';
    const typeColor = isBuy ? '#2563eb' : '#dc2626';
    const header = `${escapeHtml(row.base)} (${escapeHtml(quoteLabelFa(quote))}) | <span style="color:${typeColor};font-weight:700;">${typeLabel}</span> | ارزش: ${escapeHtml(formatQuoteValue(value, quote))}`;
    const detail = `مقدار: ${escapeHtml(formatAmount(qty))} | قیمت: ${escapeHtml(price > 0 ? formatPrice(price, quote) : '—')} | زمان: ${escapeHtml(fmtTime(row.timeMs))}`;
    histHtml += `
      <div class="lbk-sell-item">
  ...
  // Build open-position cards
  let openHtml = '';
  for (const row of openRows) {
    const quote = row.quote || 'usdt';
    const pnlText = row.pnlValue !== 0
      ? `${formatDailyPnlSignedValue(row.pnlValue, quote)} (${formatSignedPercent(row.pnlPercent)})`
      : '—';
    const header = `${escapeHtml(row.base)} (${escapeHtml(quoteLabelFa(quote))}) | <span style="color:#0ea37a;font-weight:700;">باز</span> | سود/زیان: ${escapeHtml(pnlText)}`;
    const detail = `مقدار: ${escapeHtml(formatAmount(row.buyQty))} | میانگین ورود: ${escapeHtml(row.buyPrice > 0 ? formatPrice(row.buyPrice, quote) : '—')} | از: ${escapeHtml(fmtTime(row.buyTimeMs))}`;
    openHtml += `
      <div class="lbk-sell-item">
        <div style="font-weight:700;">${header}</div>
        <small>${detail}</small>
  ...
  const netColor  = netPnl >= 0 ? '#0ea37a' : '#dc2626';

  const summaryLine = netPnl !== 0
    ? `سود/زیان خالص تقریبی: <b style="color:${netColor}">${formatDailyPnlSignedValue(netPnl, 'usdt')}</b>`
    : 'سود/زیان بسته‌شده‌ای محاسبه نشد.';

  const bodyHtml = `
    <div class="lbk-summary">
      تعداد سفارشات: <b>${formatFaNumber(histRows.length)}</b><br />
      جمع خرید: <b>${formatQuoteValue(buyTotal, 'usdt')}</b> | جمع فروش: <b>${formatQuoteValue(sellTotal, 'usdt')}</b><br />
      ${summaryLine}
    </div>
    <div style="font-weight:700; margin-bottom:8px;">معاملات اخیر</div>
    <div class="lbk-sell-list">
      ${histHtml || '<div style="white-space:pre-wrap;">سفارش اخیری از تاریخچه ccapi پیدا نشد.</div>'}
    </div>
    ${openRows.length ? `
      <div style="font-weight:700; margin-top:12px; margin-bottom:8px;">پوزیشن‌های باز</div>
      <div class="lbk-sell-list" style="margin-top:0;">${openHtml}</div>
    ` : ''}
    <div class="lbk-note" style="display:block;margin-top:10px;font-size:13px;">${escapeHtml(report?.note || '')}</div>
  `;

  return { title: 'لیست معاملات LBank', bodyHtml };
}
